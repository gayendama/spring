...
@Slf4j
public class    ItemReaderSync implements ItemReader<ItemToProcess>, StepExecutionListener {
    ...
    Integer MAX_ITEM_TO_LOAD = 10000 // Lots d'items à charger à partir de Selatis
    Integer MAX_ITEM_TO_PROCESS = 1 // Nombre d'items à traiter par la suite
    ...
    public ItemToProcess read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException, SpringbatchException {
        ...
        ItemToProcess.withNewSession {
            // Traitement un a un
           // List<ItemToProcess> itemToProcessList = ItemToProcess.findAll([order:'seq',  max: MAX_ITEM_TO_PROCESS, offset: offset])
            // L'ordre de tri est important pour le traitement !!!
            List<ItemToProcess> itemToProcessList = ItemToProcess.findAll("from ItemToProcess as i order by i.seq asc", [max: MAX_ITEM_TO_PROCESS, offset: offset])
    ...
    void beforeStep(StepExecution stepExecution) {
        ...
        ItemToProcess.withNewSession {
            ...
            itemsToProcessRequest.max = MAX_ITEM_TO_LOAD
            ...
            itemsToProcessRequest.data = data
            ...
            if (nbItemTraite == 0) {
                ...
                while (getItems) {
                    ...
                    HttpResponse<ItemToProcessResponse> httpResponse = aichaClient.getItemsToPrecess(apiManagerService.aichaBearerToken(), itemsToProcessRequest)
                    if (httpResponse.code() == HttpStatus.SC_OK) {
                        ...
                        if (itemToProcessResponse?.data != null && itemToProcessResponse?.data?.size() > 0) {
                            ...
                            itemToProcessResponse.data.eachWithIndex { itemToProcess, i ->
                                ...
                                // log.info(JsonOutput.toJson([method: "beforeStep", offsetItem: offsetItem, "itemToProcess.objetId": itemToProcess.objetId]))
                                seq++
                                itemToProcess.seq = seq
